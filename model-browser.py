...
def fetch_models(source: str, query: str) -> list[dict]:
    if source == "search":
        data = hub._request("GET", "/api/search/character_models", keyword=query or "vrm", count=50)
    elif source == "staff_picks":
        data = hub._request("GET", "/api/staff_picks", count=50)
    elif source == "hearts":
        data = hub._request("GET", "/api/hearts", count=50)
    elif source == "mine":
        data = hub._request("GET", "/api/account/character_models", count=50)
    elif source == "history":
        models = []
        for mid in vroid_sync.browsed_model_ids()[:80]:
            try:
...
async function load(){
 S.textContent='loading…';document.getElementById('grid').innerHTML='';
 const q=encodeURIComponent(document.getElementById('q').value);
 const r=await fetch(`/api/list?source=${source}&q=${q}`);const items=await r.json();
 if(items.error){S.textContent='error: '+items.error;return}
 const dl=document.getElementById('dl').checked, h18=document.getElementById('hideR18').checked;
 const seen=items.filter(m=>(!dl||m.downloadable)&&(!h18||!m.r18));
 S.textContent=`${seen.length} shown (${items.length} fetched — ${items.filter(m=>m.downloadable).length} downloadable)`;
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/api/list"):
            ...
            try:
                with lock:
                    models = fetch_models(source, query)
                self._json([serialize(m) for m in models])
